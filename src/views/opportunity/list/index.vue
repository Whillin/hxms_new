<template>
  <div class="opportunity-list-page art-full-height">
        ...
        <template #left>
          <ElSpace wrap>
            <span class="list-filter">
              <ElLink :underline="false" :type="listMode === 'mine' ? 'primary' : 'default'" @click="switchListMode('mine')">我的商机</ElLink>
              <ElLink :underline="false" :type="listMode === 'sub' ? 'primary' : 'default'" @click="switchListMode('sub')">下属商机</ElLink>
              <ElLink :underline="false" :type="listMode === 'today' ? 'primary' : 'default'" @click="switchListMode('today')">今日跟进商机</ElLink>
              <ElLink :underline="false" :type="listMode === 'all' ? 'primary' : 'default'" @click="switchListMode('all')">全部</ElLink>
            </span>
    ...
    <!-- 新增/编辑弹窗 -->
    <ElDialog v-model="dialogVisible" :title="editingId ? '编辑商机' : '新增商机'" width="800px" destroy-on-close>
      <ElForm ref="formRef" :model="formModel" :rules="formRules" label-width="110px">
        <ElRow :gutter="12">
          <ElCol :span="12">
            <ElFormItem label="留档日期" prop="visitDate">
              <ElDatePicker v-model="formModel.visitDate" type="date" value-format="YYYY-MM-DD" placeholder="请选择留档日期" />
            </ElFormItem>
          </ElCol>
          <ElCol :span="12">
          ...
          <ElCol :span="12">
            <ElFormItem label="商机编码" prop="opportunityCode">
              <ElInput v-model="formModel.opportunityCode" placeholder="保存时自动生成编码" disabled />
            </ElFormItem>
          </ElCol>
          <ElCol :span="12">
          ...
          <ElCol :span="12">
            <ElFormItem label="居住区域" prop="livingArea">
              <ElCascader v-model="formModel.livingArea" :options="cityCascaderOptionsRef" :props="{ value: 'label', label: 'label', children: 'children' }" placeholder="请选择省/市/区" />
            </ElFormItem>
          </ElCol>
          <ElCol :span="24">
            <ElFormItem label="客户描述" prop="customerDesc">
              <ElInput v-model="formModel.customerDesc" type="textarea" rows="3" placeholder="请输入客户描述" />
            </ElFormItem>
          </ElCol>
          <ElCol :span="12">
          ...
          <ElCol :span="12">
            <ElFormItem label="战败/未成交分析" prop="defeatReasons" :required="formModel.latestStatus !== '已成交'">
              <ElSelect v-model="formModel.defeatReasons" multiple collapse-tags :disabled="formModel.latestStatus === '已成交'" placeholder="请选择原因">
          ...
          <ElCol :span="24">
            <ElFormItem label="备注" prop="remark">
              <ElInput v-model="formModel.remark" type="textarea" rows="2" placeholder="请输入备注" />
            </ElFormItem>
          </ElCol>
        </ElRow>
      </ElForm>
        ...
        <div class="follow-form">
          ...
          <ElForm ref="followFormRef" :model="followForm" :rules="followRules" label-width="110px">
            <ElFormItem label="跟进内容" prop="content">
              <ElInput v-model="followForm.content" type="textarea" rows="3" placeholder="请输入跟进内容" />
            </ElFormItem>
            <ElFormItem label="下次联系时间" prop="nextContactTime">
              <ElDatePicker v-model="followForm.nextContactTime" type="datetime" value-format="YYYY-MM-DD HH:mm:ss" placeholder="选择下次联系时间" />
            </ElFormItem>
...
const searchItems = computed(() => [
  ...
  { label: '一级渠道', key: 'channelLevel1', type: 'select', props: { options: channelLevel1Options } },
  { label: '关注车型', key: 'focusModelName', type: 'select', props: { options: nameOptions.value } },
  { label: '商机级别', key: 'opportunityLevel', type: 'select', props: { options: opportunityLevelOptions } },
  {
    label: '试驾',
    key: 'testDrive',
    type: 'select',
    props: { options: [ { label: '否', value: false }, { label: '是', value: true } ] }
  },
  {
    label: '议价',
    key: 'bargaining',
  ...
  { label: '购车经历', key: 'buyExperience', type: 'select', props: { options: buyExperienceOptions } },
  ...
  { label: '最新状态', key: 'latestStatus', type: 'select', props: { options: [
    { label: '跟进中', value: '跟进中' },
    { label: '已战败', value: '已战败' },
    { label: '已成交', value: '已成交' }
  ] } },
  { label: '战败/未成交分析', key: 'defeatReasons', type: 'select', props: { multiple: true, collapseTags: true, options: defeatReasonOptions } },
...
const mockApi = async (
  params: any
): Promise<Api.Common.PaginatedResponse<OpportunityItem>> => {
  ...
  if (params.salesConsultant) filtered = filtered.filter((r) => (r.salesConsultant || '').includes(params.salesConsultant))
  if (params.customerName) filtered = filtered.filter((r) => (r.customerName || '').includes(params.customerName))
  if (params.customerPhone) filtered = filtered.filter((r) => (r.customerPhone || '').includes(params.customerPhone))
  if (params.opportunityCode) filtered = filtered.filter((r) => (r.opportunityCode || '').includes(params.opportunityCode))
  if (params.channelLevel1) filtered = filtered.filter((r) => r.channelLevel1 === params.channelLevel1)
  if (params.focusModelName) filtered = filtered.filter((r) => r.focusModelName === params.focusModelName)
  if (params.opportunityLevel) filtered = filtered.filter((r) => r.opportunityLevel === params.opportunityLevel)
  if (params.testDrive !== undefined && params.testDrive !== null && params.testDrive !== '')
    filtered = filtered.filter((r) => r.testDrive === (params.testDrive === true || params.testDrive === 'true'))
  if (params.bargaining !== undefined && params.bargaining !== null && params.bargaining !== '')
    filtered = filtered.filter((r) => r.bargaining === (params.bargaining === true || params.bargaining === 'true'))
  if (params.buyExperience) filtered = filtered.filter((r) => r.buyExperience === params.buyExperience)
  if (params.currentModel) filtered = filtered.filter((r) => (r.currentModel || '').includes(params.currentModel))
  ...
  if (Array.isArray(params.livingArea) && params.livingArea.length) {
    ...
    filtered = filtered.filter((r) => (Array.isArray(r.livingArea) ? r.livingArea.join('/') : '').startsWith(target))
  }
  if (params.latestStatus) filtered = filtered.filter((r) => r.latestStatus === params.latestStatus)
  if (Array.isArray(params.defeatReasons) && params.defeatReasons.length)
    filtered = filtered.filter((r) => params.defeatReasons.every((v: string) => r.defeatReasons.includes(v)))
...
const {
  ...
  core: {
    apiFn: async ({ current, size }: Api.Common.CommonSearchParams): Promise<Api.Common.PaginatedResponse<OpportunityItem>> => {
    ...
    columnsFactory: (): ColumnOption<OpportunityItem>[] => [
      ...
      { prop: 'testDrive', label: '试驾', width: 90, formatter: (_r, _c, v) => (v ? '是' : '否') },
      { prop: 'bargaining', label: '议价', width: 90, formatter: (_r, _c, v) => (v ? '是' : '否') },
      { prop: 'buyExperience', label: '购车经历', width: 100 },
      { prop: 'currentModel', label: '现用车型', minWidth: 120 },
      { prop: 'carAge', label: '车龄(年)', width: 100 },
      { prop: 'livingArea', label: '居住区域', minWidth: 180, formatter: (row, _c, v) => (Array.isArray(v) ? v.join('/') : v) },
      { prop: 'customerDesc', label: '客户描述', minWidth: 180 },
      { prop: 'latestStatus', label: '最新状态', width: 110 },
      { prop: 'defeatReasons', label: '战败/未成交分析', minWidth: 200, formatter: (_r, _c, v) => Array.isArray(v) ? v.join('、') : v },
      { prop: 'remark', label: '备注', minWidth: 160 },
      { prop: 'operation', label: '操作', width: 200, align: 'center', fixed: 'right', useSlot: true }
    ]
  }
})
  ...
  defeatReasons: [
    {
      validator: (_: any, value: any, cb: any) => {
        if (formModel.value.latestStatus !== '已成交' && (!value || (Array.isArray(value) && value.length === 0))) {
...
const submitForm = async () => {
  ...
  const payload: OpportunityItem = {
    ...
    livingArea: Array.isArray(formModel.value.livingArea)
      ...
      : String(formModel.value.livingArea || '').split('/').filter(Boolean),
    defeatReasons:
      formModel.value.latestStatus === '已成交' ? [] : (Array.isArray(formModel.value.defeatReasons) ? formModel.value.defeatReasons : [])
  }
  if (!editingId.value) {
...
const followForm = ref<{ content: string; nextContactTime: string; status: string; method: string }>({
  content: '',
  nextContactTime: '',
  status: '跟进中',
  method: '电话'
...
const {
  data: followTableData,
  columns: followColumns,
  columnChecks: _followColumnChecks,
  loading: followLoading,
  pagination: followPagination,
  ...
  core: {
    apiFn: async ({ current, size }: Api.Common.CommonSearchParams): Promise<Api.Common.PaginatedResponse<FollowUpRecord>> => {
      const all = followRecords.value.filter((r) => r.opportunityId === currentFollowOpportunityId.value)
    ...
    columnsFactory: (): ColumnOption<FollowUpRecord>[] => [
      ...
      { prop: 'operation', label: '操作', width: 140, align: 'center', fixed: 'right', useSlot: true }
    ]
  }
})
</script>
