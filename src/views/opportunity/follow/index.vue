...
const searchItems = computed(() => [
  { label: '跟进方式', key: 'method', type: 'select', props: { options: followMethodOptions, placeholder: '请选择跟进方式' } },
...
const {
  ...
  core: {
    apiFn: async ({ current, size }: Api.Common.CommonSearchParams): Promise<Api.Common.PaginatedResponse<FollowUpRecord>> => {
      ...
      if (kw) {
        all = all.filter((r) => (r.opportunityName || '').includes(kw) || (r.content || '').includes(kw))
      }
      // 按创建时间倒序
