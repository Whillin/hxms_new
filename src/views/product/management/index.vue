  ...
  <div class="page-content product-management">
    <ElCard shadow="never">
      ...
      <div class="search-form">
        <ElRow :gutter="20">
          ...
          <ElCol :span="10">
            ...
            <ElButton type="success" @click="handleBatchExport" v-auth="'export'" :disabled="selectedRows.length === 0">
      ...
      >
        ...
        <ElTableColumn prop="categoryName" label="车型分类" width="150" />
        ...
        <ElRow :gutter="20">
          ...
          <ElCol :span="16">
            <ElDescriptions :column="2" border>
              ...
              <ElDescriptionsItem label="车型分类">{{ currentProduct.categoryName }}</ElDescriptionsItem>
    ...
    >
      ...
      >
        <ElRow :gutter="20">
          ...
          <ElCol :span="12">
            <ElFormItem label="车型分类" prop="categoryId">
              <ElSelect
...
const categoryOptions = ref([
  { id: 1, name: '奥迪', children: [
    { id: 2, name: 'CKD(ICE)', parentName: '奥迪' },
    { id: 3, name: 'NEV', parentName: '奥迪' },
    { id: 4, name: 'FBU', parentName: '奥迪' }
  ]},
  { id: 5, name: '小鹏', children: [
    { id: 6, name: 'NEV', parentName: '小鹏' }
  ]}
])
...
const flatCategoryOptions = computed(() => {
  const result: any[] = []
  categoryOptions.value.forEach(brand => {
    if (brand.children) {
      brand.children.forEach(category => {
        result.push({
          id: category.id,
...
const addFormRules = {
  ...
  categoryId: [{ required: true, message: '请选择车型分类', trigger: 'change' }],
  price: [
...
const handleAddSubmit = async () => {
  try {
    ...
    // 获取选中分类的品牌信息
    const selectedCategory = flatCategoryOptions.value.find(cat => cat.id === addForm.categoryId)
    if (selectedCategory) {
