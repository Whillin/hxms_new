    ...
    <el-card class="art-custom-card" style="margin-bottom: 12px">
      <div class="filters">
        ...
        <el-switch
          ...
          style="margin-left: 8px"
        />
        <el-switch v-model="enableCompare" size="small" style="margin-left: 8px" />
    ...
    <div v-else>
      <el-card class="art-custom-card">
        ...
        <div class="legend">
          ...
          <span class="legend-item"><i class="legend-dot dot-b"></i>{{ legendB }}</span>
        </div>
...
<script setup lang="ts">
  ...
  const tableRowsCombined = computed(() => {
    ...
    return funnelItems.value.map((itA) => {
      const stage = String(itA.stage)
  ...
  const loadFunnel = async () => {
    ...
    if (channelType.value) void channelType.value
    ...
    if (periodType.value === 'month' && dateMonth.value) params.month = dateMonth.value
    if (periodType.value === 'year' && dateYear.value) params.year = dateYear.value

    const res = await request.get<{ items: { stage: string; value: number }[] }>({
      url: '/api/bi/sales-funnel',
      params
    })
    const itemsA = Array.isArray(res?.items) ? res.items : []
    funnelItems.value = itemsA
  ...
  const loadFunnelRight = async () => {
    ...
    funnelItemsRight.value = itemsB
  }

  const loadPrevFunnel = async (baseParams: Record<string, any>) => {
    const params2: Record<string, any> = { ...baseParams }
    if (periodType.value === 'month') {
      const cur = String(dateMonth.value || '')
      const d = cur ? new Date(`${cur}-01T00:00:00`) : new Date()
      d.setMonth(d.getMonth() - 1)
      const prev = `${d.getFullYear()}-${String(d.getMonth() + 1).padStart(2, '0')}`
      params2.month = prev
      delete params2.start
      delete params2.end
      const res2 = await request.get<{ items: { stage: string; value: number }[] }>({
        url: '/api/bi/sales-funnel',
        params: params2
      })
      const map: Record<string, number> = {}
      ;(Array.isArray(res2?.items) ? res2.items : []).forEach((it) => {
        map[String(it.stage)] = Number(it.value) || 0
      })
      prevMap.value = map
    } else {
      prevMap.value = {}
  ...
  const tableRows = computed(() => {
    ...
    return funnelItems.value.map((it) => {
  ...
  const tableRowsRight = computed(() => {
    ...
    return funnelItemsRight.value.map((it) => {
      const stage = String(it.stage)
      const v = Number(it.value) || 0
      const baseStageMap: Record<string, string | undefined> = {
        全部订单数量: undefined,
      ...
      const p = Math.min(100, Math.round(pRaw))
      const percentText = Math.min(100, Math.round(pRaw * 10) / 10).toFixed(1)
      return { stage, value: v, percent: p, percentText }
    })
  })
