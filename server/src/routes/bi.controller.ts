...
export class BiController {
  ...
  async salesFunnel(@Req() req: any, @Query() query: any) {
    ...
    qbOpp.andWhere('LOWER(TRIM(o.channelLevel1)) IN (:...l1)', { l1: l1Arr })
    const l2Lower = String(qL2 || '').trim().toLowerCase()
    ...
    const phones = Array.from(new Set(opps.map((o) => String(o.customerPhone || ''))))
    const storesToQuery = storeId ? [storeId] : allowed.length ? allowed : []

    const qbClue = this.clueRepo.createQueryBuilder('c')
    qbClue.where('1=1')
    if (storesToQuery.length) qbClue.andWhere('c.storeId IN (:...stores)', { stores: storesToQuery })
    qbClue.andWhere('c.visitDate BETWEEN :start AND :end', { start: String(start), end: String(end) })
    qbClue.andWhere('c.channelCategory = :online', { online: '线上' })
    qbClue.andWhere('LOWER(TRIM(c.channelLevel1)) IN (:...l1)', { l1: l1Arr })
    ...
    if (storesToQuery.length) {
      ...
      qbDaily.andWhere('d.date BETWEEN :start AND :end', { start: String(start), end: String(end) })
    ...
    const combinedTestDrivesRaw = Math.max(0, Number(firstTestDriveCount || 0)) + Math.max(0, Number(againTestDriveCount || 0))
    ...
    const valueMap: Record<string, number> = items.reduce((acc, it) => {
